 ...
 * @apiGroup PillarData
 * @apiName  0
 * @apiDescription 单个柱子类
 * @apiParam (成员变量) m_vPos 柱子位置
 */
function PillarData()
{
	 	this.m_vPos;
		...
		this.OnCreate2D = function()
		{
			...
			var groundMaterial  = new THREE.MeshBasicMaterial({color: 0xBBBBBB, opacity: 1, transparent: true});		
			this.m_RenderData2D = new THREE.Mesh(groundGeometry, groundMaterial);								
			scene.add(this.m_RenderData2D);

			this.BuildPillarIcon(0);
		};
	...
	//		result_poly.vertices.push(new THREE.Vector3( tLength/4, tWidth/3, 1.3), new THREE.Vector3(-tLength/2+3, tWidth/2-3, 1.3));
		...
		this.OnCreatePlane = function(pos1,pos2,iTextureData)
		{
				...
				geom.vertices.push(new THREE.Vector3(pos1.x, pos1.y, 0));
				geom.vertices.push(new THREE.Vector3(pos2.x, pos2.y, 0));
