...
outerHeight(".template-listInfo-typ",".main-body",".templateRoom .pagechang",-42);
outerWidth(".modelBase-right",".modelBase",".modelBase-left_block");
...
// 模型库类型模块点击事件
	...
	$('.modelBase-left>li').click(function(){
		...
		else if(this.id == "502") //企业库
		{
			$("#gerenku_model").show();
			Mods_Arr=grk_data.m_ObjGroup;
		}		
...
$(".modelBase_Mat_model .modelBase_Mat_scrollbar").delegate("li",'click',function(){
	...
	$('.modelBase-left>li').removeClass("modelBase_link");
	$('#mMaterialDlg').show();
	$('.modelBase-left>li').eq(2).addClass("modelBase_link");
...
// 切换视图
	...
	$('.modelBase-right ul').delegate('.icon-xinxi','click',function(){
		let itemId="";
		if (m_version=='index_3') {
			itemId=$(this).siblings('img').attr('id');
		}else{
			itemId=$(this).next().attr('id');
			if (itemId==undefined) {
...
// 左侧弹出框
	...
	$('#mTool').undelegate('.itemList-title','click').delegate('.itemList-title','click',function(){
