//chenx 新增
var mFurniture_ObjControls;	// 当前家具
THREE.ObjControls = function ( camera, domElement ) {
	...
	var _gizmo = new THREE.ObjControlsGizmo();	// 线框
	this.add( _gizmo );

	var _plane = new THREE.ObjControlsPlane();
	this.add( _plane );
	...
	// Set current object
	this.attach = function ( object, tFurniture ) {
		mFurniture_ObjControls = tFurniture;
		this.object = object;
		this.visible = true;
	};
		...
		Object.defineProperty( scope, propName, {
			...
			set: function ( value ) {
				...
				if ( propValue !== value ) {
					...
					scope.dispatchEvent( changeEvent );	// Render更新
				}
			}
		} );
	...
	this.pointerMove = function ( pointer ) {
		...
		if ( mode === 'translate' ) {
			...
			if ( axis === 'R' ) {
				...
				object.rotation.z += rotationAngle;// 累加旋转	
				
				mFurniture_ObjControls.m_fRotate = object.rotation.z*180/Math.PI;
			}
		}
	...
	this.update = function () {
		console.warn( 'THREE.ObjControls: update function has been depricated.' );
	};

};

THREE.ObjControls.prototype = Object.assign( Object.create( THREE.Object3D.prototype ), {

	constructor: THREE.ObjControls,
...
THREE.ObjControlsGizmo = function () {

	'use strict';

	THREE.Object3D.call( this );

	this.type = 'ObjControlsGizmo';
	...
	var gizmoTranslate = {
		...
		XY: [ //chenx 0.295 改为 0.395
			[ new THREE.Mesh( new THREE.PlaneBufferGeometry( 0.395, 0.395 ), matYellowTransparent ), [ 0.15, 0.15, 0 ]] // 0.15,0.15,0 为位置
		],
		YZ: [ //chenx 0.295 改为 0.395
			[ new THREE.Mesh( new THREE.PlaneBufferGeometry( 0.395, 0.395 ), matCyanTransparent ), [ 0, 0.15, 0.15 ], [ 0, Math.PI / 2, 0 ]]
		],
		XZ: [ //chenx 0.295 改为 0.395
			[ new THREE.Mesh( new THREE.PlaneBufferGeometry( 0.395, 0.395 ), matMagentaTransparent ), [ 0.15, 0, 0.15 ], [ - Math.PI / 2, 0, 0 ]]
		],
		R: [
		...
		//	[ new THREE.Mesh( new THREE.TorusBufferGeometry( 1, 0.06, 8,16,6.3 ), matLineGreen ), null, [ - Math.PI / 2, 0, - Math.PI / 2 ]],
	...
	var pickerTranslate = {
		...
		//	[ new THREE.Line( CircleGeometry( 1, 1 ), matRed ), [ 0, 0, 0 ], [ 0, 0, - Math.PI / 2 ]],
			[ new THREE.Mesh( new THREE.TorusBufferGeometry( 1, 0.06, 8,16,6.3 ), matLineGreen ), null, [ - Math.PI / 2, 0, - Math.PI / 2 ]],
		...
		for ( var i = 0; i < handles.length; i ++ ) {
			...
			if ( this.mode === 'translate' ) {
				...
				if ( handle.name === 'R' ) {
					...
					tempQuaternion.setFromAxisAngle( unitY, Math.atan2( alignVector.x, alignVector.y ) );
...
THREE.ObjControlsGizmo.prototype = Object.assign( Object.create( THREE.Object3D.prototype ), {

	constructor: THREE.ObjControlsGizmo,
	isTransformControlsGizmo: true

} );

THREE.ObjControlsPlane = function () {
...
THREE.ObjControlsPlane.prototype = Object.assign( Object.create( THREE.Mesh.prototype ), {
	constructor: THREE.ObjControlsPlane,
