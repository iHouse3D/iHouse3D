...
THREE.TDSLoader.prototype = {
	 ...
	 */
	//最初接口名称为load,这里作为备份增加了_old(这个接口通过url加载)
	...
	//通过后台接口加载模型
	load_new: function ( url, onLoad, onProgress, onError ) {
		var scope = this;
		var path = this.path !== undefined ? this.path : THREE.LoaderUtils.extractUrlBase( url );
		var loader = new THREE.FileLoader( this.manager );
		loader.setPath( this.path );
		...
		let modelFile = url;
		modelFile = url.replace(m_strWebService,"");
		this.OnDownload(modelFile,path,onLoad,scope,onError);
	},
	OnDownload:function(modelFile,path,onLoad,scope,onError)
	{
		$.ajax({
			url:'http://www.ihouse3d.com.cn/Service1.asmx/DownloadBinaryFile',
			type: "post",
			dataType:"json",
			...
			success:function(data){
				...
				{
					//data.data为后台返回的base64模型数据（这里需要转换为原模型数据）
					var modelData = Base64Binary.decodeArrayBuffer(data.data);
					onLoad( scope.parse(modelData, path ) );
				}
			},
			error: function (err)
			{
	...
	parse: function ( arraybuffer, path ) {
		...
		if(this.meshes.length==1)	// 两个以上不确定怎么处理
		{
			this.buildSubMesh(0);
		}
		else
		{
			...
			//	this.buildSubMesh(i);
			var material = this.materials[ this.groups[i].name ];
	...
	//chenx 6  按groups记录的faceID , 重新创建各个子mesh  
	buildSubMesh: function ( t_index ) {
		
		var tPosition = this.meshes[t_index].geometry.getAttribute( 'position' );
		var tUV		  = this.meshes[t_index].geometry.getAttribute( 'uv' );
		for ( var i = 0; i < this.groups.length; i ++ ) 
		{
			...
			var mesh 	 = new THREE.Mesh( geometry, material );
				mesh.name='mesh';
			var iCount = 0;
			for( var j = 0; j<this.groups[i].index.length; j++ )
			{
				 var a = this.meshes[t_index].geometry.index.array[this.groups[i].index[j]*3+0];
				 var b = this.meshes[t_index].geometry.index.array[this.groups[i].index[j]*3+1];
				 var c = this.meshes[t_index].geometry.index.array[this.groups[i].index[j]*3+2];
				 
				vertices.push( tPosition.getX( a ));
				vertices.push( tPosition.getY( a ));
				vertices.push( tPosition.getZ( a ));

				vertices.push( tPosition.getX( b ));
				vertices.push( tPosition.getY( b ));
				vertices.push( tPosition.getZ( b ));	

				vertices.push( tPosition.getX( c ));
				vertices.push( tPosition.getY( c ));
				vertices.push( tPosition.getZ( c ));
			...
			//X Line
			matrix.elements[ 0 ] = this.values[ 0 ];
			matrix.elements[ 1 ] = this.values[ 6 ];
			matrix.elements[ 2 ] = this.values[ 3 ];
			matrix.elements[ 3 ] = this.values[ 9 ];

			//Y Line
			matrix.elements[ 4 ] = this.values[ 2 ];
			matrix.elements[ 5 ] = this.values[ 8 ];
			matrix.elements[ 6 ] = this.values[ 5 ];
			matrix.elements[ 7 ] = this.values[ 11 ];

			//Z Line
			matrix.elements[ 8 ] = this.values[ 1 ];
			matrix.elements[ 9 ] = this.values[ 7 ];
			matrix.elements[ 10 ] = this.values[ 4 ];
			matrix.elements[ 11 ] = this.values[ 10 ];
	...
	readMaterialEntry: function ( data, path ) {
		...
		var chunk = this.readChunk( data );
		var next = this.nextChunk( data, chunk );
	//  var material = new THREE.MeshPhongMaterial();	//chenx
	//	var material = new THREE.MeshStandardMaterial();
		var material = new THREE.MeshLambertMaterial(); //chenx 
		...
		while ( next !== 0 ) {
			...
			if ( next === POINT_ARRAY ) {
				...
				for ( var i = 0; i < 12; i ++ ) {
					...
					this.values[i]=values[i];		//chenx 9
				}
...
//chenx 4  while ( this.position < chunk.end ) {
		 while ( this.position < this.end ) {
			...
			if ( chunk.id === MSH_MAT_GROUP ) {
				...
				this.groups.push( group );			//chenx 5
	...
	readMap: function ( data, path ) {
		...
		while ( next !== 0 ) {
			...
			if ( next === MAT_MAPNAME ) {
				...
				//chen 替换  ，加载不到贴图时提示，贴图错误
				texture = loader.load( name );
			//	texture = loader.load( name ,undefined,undefined,function ( err ) {
			//		alert(name+",下载不到.");
			//	} );
				texture.wrapS = texture.wrapT = THREE.RepeatWrapping;	//chenx
